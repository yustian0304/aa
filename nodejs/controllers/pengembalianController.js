...
exports.index = function(req, res) {
  ...
  db.query("SELECT a.*,b.nama_anggota FROM peminjaman a "+
    ...
    if (err) throw err;
  		res.render('layout',{
        'render_view'	 :{'model':'pengembalian','view':'index','msg':req.flash('info')},
...
exports.book_update_get = function(req, res) {
  ...
  var id=req.params.id;
    ...
    if (err) throw err;
      db.query("select a.*,coalesce((a.jumlah-(select sum(jumlah_kembali) from pengembalian where id_buku=a.id_buku and id_peminjaman=a.id_peminjaman)),a.jumlah) as result_jumlah,c.pengarang,c.penerbit from peminjaman_detail a"+
      ...
      if (err) throw err;
              ...
              res.render('layout',{
                    'render_view'     :{'model':'pengembalian','view':'update','msg':req.flash('info')},
                    'result'          :JSON.stringify(result),
...
exports.book_update_post = function(req, res) {
  ...
  //        if(typeof req.body.id_detail!='undefined'){
         ...
         // sql_detail="update peminjaman_detail set jumlah_kembali='"+jumlah[i]+"' where id='"+id_detail[i]+"'";
         //   db.query(sql_detail, function (err, result) {
         //      if (err) throw err;
         //   });
         // }
  ...
  //        if(typeof req.body.id_baru!='undefined'){
         for (var i = 0; i < req.body.id_buku.length; i++) {
         var sql_insert_detail="insert into pengembalian (id_buku,id_peminjaman,judul_buku,jumlah,jumlah_kembali) values ('"+req.body.id_buku[i]+"','"+id+"','"+req.body.judul_buku[i]+"','"+req.body.jumlah[i]+"','"+req.body.jumlah[i]+"')";
